...
- Jupyter Notebook as workspace

**Status: complete.** All six variables acquired and verified — `panel`, `repo_daily`, `m3_daily`, `term_spread`, `us10y_daily`, `usdinr_daily`.
...
- Assemble final input matrix: G-Sec yields across maturities + repo rate + M3 growth + interest rate spread + foreign rate + forward premium proxy, aligned to a common (weekly) frequency.
...
- Python: `pandas`, `numpy` (manual eigendecomposition for understanding), `sklearn.decomposition.PCA` (for convenience once done manually once), `matplotlib`/`seaborn` for loading plots
- Jupyter Notebook

**Status: complete.** See `Week2_Inference_Report.md` for full interpretation of PC1–PC3 and variance explained.
...
- Compute RMSE for the model vs. random-walk benchmark, per component and per horizon.
...
- Word or a markdown file for the non-technical summary (Excel not required anywhere in this pipeline; only optional for a polished business-facing chart export at the very end)

**Status: not started.**

---

## Full tool stack across the month

- **Python/Jupyter** — the entire technical pipeline: `pandas`, `numpy`, `sklearn`, `statsmodels`, `matplotlib`/`seaborn`. Used every week.
- **Browser** — manual data pulls from CCIL, RBI DBIE, RBI's monetary policy page, FRED — front-loaded mostly in Week 1, occasional revisits if backfilling is needed.
